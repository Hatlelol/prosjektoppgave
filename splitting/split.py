...
def writeTiles(tiles, metadata, overlap, inp_shape, folder, origin="UL", noise_types=None, overlap_=None, shift=None):
    """
    ...
    if origin == "LL":
        ...
        c_x = lambda x, c:x
        if rows > 1:
            c_y = lambda y, c: y - metadata[rows-1, 0][0]
        else:
            c_y = lambda y, c: -y
    elif origin == "UR":
        x = lambda x:cols - x - 1
    ...
    # print(metadata)
    # print(metadata - metadata[y(0), x(0)])
    # print(np.abs(metadata- metadata[y(0), x(0)]))
    metadata = metadata - metadata[y(0), x(0)]
    print(metadata[:, :, 0] )
    ...
    for i in range(tiles.shape[0]):
        for j in range(tiles.shape[1]):
            ...
            out_string          += f"{folder}\img_{i:03}_{j:03}.tif ;; ({coords[0]}, {coords[1]})\n"
            out_string_altered  += f"{folder}\img_{i:03}_{j:03}.tif ;; ({coords[0] + y_random[i, j]}, {coords[1] + x_random[i, j]})\n"
...
def main(folder_name, rows, cols, overlap_p, shift_w, shift_h, origin, random_shift, noise=False):
    ...
    tiles, metadata, noise_types, overlap_w, overlap_h = splitImage(source_image, overlap_p, rows, cols, shift_w, shift_h, randomShift=random_shift, noise=noise)
    print(f"Time used for splitting {time.time() - start_time} ")
    start_time = time.time()

    
    save_folder = createFolder(folder, f"split_{rows}_{cols}")
    writeTiles(tiles, metadata, overlap_p, source_image.shape, save_folder, origin, noise_types, [overlap_w, overlap_h], [shift_w, shift_h])
...
if __name__ == "__main__":
    ...
    # R"C:\data\space\CosmicCliffs\split_5_5",
    # R"C:\data\space\SouthernRing\split_3_5",
    # R"C:\data\space\Stephans_Quintet\split_5_5",
    # R"C:\data\space\DeepField\split_3_3"
    folder_name = "SouthernRing" 
    rows = 3
    cols = 5 
    overlap_p = 15 
    shift_h = 30 
    ...
    if folder_name != None:
        if rows != None:
            if cols != None:
                ...
                print(f"Running split progran with parameters {folder_name} {rows} {cols} {overlap_p} {shift_w} {shift_h} {origin} {random_shift} {noise}")

                main(folder_name=folder_name, rows=rows, cols=cols, overlap_p=overlap_p, shift_w=shift_w, shift_h=shift_h, origin=origin, random_shift=random_shift, noise=noise)
                
            else:
                print("No cols are defined, commands is folder_name, rows, cols, overlap_p, shift_w, shift_h, origin")
        else:
            print("No rows are defined, commands is folder_name, rows, cols, overlap_p, shift_w, shift_h, origin")
    else:
        print("No folder name is defined, commands is folder_name, rows, cols, overlap_p, shift_w, shift_h, origin")
