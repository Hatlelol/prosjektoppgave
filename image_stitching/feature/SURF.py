...
class SURF(stitch):
    ...
    def __init__(self, image1: np.ndarray, image2: np.ndarray,  position: np.ndarray = None, repeatability: int = None, method = "NARROW") -> None:
        """
        class to run SURF, Currently not working as SURF is nonfree
    ...
    def stitch(self):
        ...
        surf.setExtended(True) #set longer dimentions for discriptors
        surf.setUpright(True)
        
        # Find keypoints and descriptors directly
        kp1, des1 = surf.detectAndCompute(self.image1, None)
        end_kp1 = time.time()
        kp2, des2 = surf.detectAndCompute(self.image2, None)
        end_kp2 = time.time()
